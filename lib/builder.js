  ...
  if (typeof fieldDefinition == 'object') {
    ...
    for (var validatorKey in fieldDefinition) {
      if (fieldDefinition.hasOwnProperty(validatorKey)) {
        ...
        var validatorDefinition = fieldDefinition[validatorKey];
        var generatedValidator;
        if (util.isArray(validatorDefinition)) {
          generatedValidator = validator.apply(null, fieldDefinition[validatorKey]);
        } else{
          generatedValidator = validator();
        }

        fieldValidators.push({ name : validatorKey, validate : generatedValidator });
      }
    }
